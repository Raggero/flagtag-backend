...
app.post(
	'/users',
	body('username', 'The username must be minimum 2 characters').isLength({
		min: 2,
	}),
	body('password', 'The password must be minimum 2 characters').isLength({
		min: 2,
	}),
	(req, res, next) => {
...
app.put(
	'/users/update',
	body('newUser', 'The username must be minimum 2 characters').isLength({
		min: 2,
	}),
	(req, res, next) => {
		let errors = validationResult(req)
...
app.get('/highscore', (req, res, next) => {
	const searchQuery =
		'SELECT * FROM users WHERE userId >1 ORDER BY highscoreAllRegions desc LIMIT 5'
	let params = []
...
app.get('/highscore/:id', (req, res) => {
	const searchQuery =
		'SELECT userId, highScoreAllRegions, highScoreAfrica, highScoreAmericas, highScoreAsia, highScoreEurope, highScoreOceania FROM users WHERE userId = (?)'
...
app.put('/highscore/:name', (req, res, next) => {
	const searchQuery = 'select * from users where userName = ?'
	const updateQuery =
		'UPDATE users SET highScoreAllRegions = 0 WHERE userName = ?'
	let params = req.params.name
	let accountExists
